...
func _ready() -> void:
	...
	color_choice.item_selected.connect(update_key_color.unbind(1))
	...
	type_choice.item_selected.connect(update_key_type.unbind(1))
	...
	real_amount.value_changed.connect(update_key_amount.unbind(1))
	imaginary_amount.value_changed.connect(update_key_amount.unbind(1))

func update_key_color() -> void:
	key.key_data.color = color_choice.get_item_id(color_choice.selected)

func update_key_type() -> void:
	key.key_data.type = type_choice.get_item_id(type_choice.selected)
	amount.visible = key.key_data.type in [Enums.key_types.add, Enums.key_types.exact]

func update_key_amount() -> void:
